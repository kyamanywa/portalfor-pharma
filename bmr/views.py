...
def bmr_detail_view(request, bmr_id):
    ...
    context = {
        ...
        'qms_users': CustomUser.objects.filter(role__in=['qa', 'head_qa', 'qc']).order_by('first_name', 'last_name'),
    }
...
def bmr_issuance_log_detail(request, log_id):
    ...
    # Get the first entry to determine active ingredient columns
    active_ingredient_columns = []
    if entries.exists():
        first_entry = entries.first()
        active_ingredient_columns = first_entry.active_ingredients
