...
def bmr_detail_view(request, bmr_id):
    """
    Canonical BMR detail entrypoint.

    This route is retained for backward compatibility, but rendering is unified
    through dashboards.phase_form_view to avoid context drift between duplicate
    BMR render paths.
    """
    ...
    if not anchor_execution:
        messages.error(request, 'No phase execution found for this BMR.')
        return redirect('bmr:list')

    target_url = reverse('dashboards:phase_form', args=[anchor_execution.id])
