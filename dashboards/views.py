...
def qa_dashboard(request):
    ...
    if request.method == 'POST':
        ...
        # Handle Final QA workflow actions
        if phase_id and action in ['start', 'approve', 'reject']:
            try:
                ...
                if action == 'start':
                    ...
                    # Use automatic rollback service
                    rollback_success = WorkflowService.handle_qc_failure_rollback(
                        bmr, 'final_qa', rollback_phase
                    )
                    ...
                    if rollback_success:
                        ...
                        messages.error(request, f'Error rolling back batch {bmr.batch_number}. Please contact administrator.')
                    
            except Exception as e:
                messages.error(request, f'Error processing Final QA: {str(e)}')
        
        # Handle rejected BMR re-submission
        elif bmr_id and action == 'resubmit_rejected':
            try:
                bmr = get_object_or_404(BMR, pk=bmr_id, status='rejected')
                
                # Use service method to re-submit rejected BMR
                success = WorkflowService.resubmit_rejected_bmr(
                    bmr, 
                    qa_comments=f"Re-submitted by {request.user.get_full_name()}. QA Review: {comments}"
                )
                
                if success:
                    messages.success(request, f'BMR {bmr.batch_number} has been re-submitted to Regulatory for review.')
                else:
                    messages.error(request, f'Failed to re-submit BMR {bmr.batch_number}. Please contact administrator.')
    ...
    _secondary_anchor_map = {
        'sec_inspection_sorting': 'sec-sec-inspection_sorting',
        'sec_packing_procedure': 'sec-sec-packing_procedure',
        'sec_shipper_weight': 'sec-sec-shipper_weight',
        'sec_ipc_p54': 'sec-sec-ipc_p54',
        'sec_ipc_p55': 'sec-sec-ipc_p55',
        'sec_ipc_p56': 'sec-sec-ipc_p56',
        'sec_fp_recon': 'sec_fp_recon',
        # Fallback to the secondary block start when coding anchor is unavailable.
        'sec_packing_coding': 'sec-sec-beginning_lc',
    }
            ...
            if _skey == 'sec_packing_coding':
                ...
                if _cod_stage == 'coding_submitted':
                    pending_secondary_signing.append({
                        ...
                        'bmr': _spe.bmr,
                        'section_key': _skey,
                        'anchor': _secondary_anchor_map.get(_skey, ''),
                ...
                if _fpr_stage_tablet == 'spv_submitted':
                    ...
                    pending_secondary_signing.append({
                        ...
                        'anchor': _secondary_anchor_map.get('sec_fp_recon', ''),
            ...
            if _ss2.get(_sipc_key) == 'in_progress':
                ...
                if _sipc_d.get('next_turn') == 'qa':
                    ...
                    pending_secondary_signing.append({
                        ...
                        'anchor': _secondary_anchor_map.get(_sipc_key, ''),
